...
if exercise_right == 1:
    st.markdown(f"<div style='text-align: center; font-size: x-small;'><b>Put probabilities values based on the analyzed historical date (from {start_date} to now)</b></div>", unsafe_allow_html=True)
else:
    st.markdown(f"<div style='text-align: center; font-size: x-small;'><b>Call probabilities values based on the analyzed historical date (from {start_date} to now)</b></div>", unsafe_allow_html=True)
