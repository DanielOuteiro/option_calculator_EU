...
if exercise_right == 1:
    st.markdown("<div style='text-align: center;'><h2><b>PUT</b></h2></div>", unsafe_allow_html=True)
else:
    st.markdown("<div style='text-align: center;'><h2><b>CALL</b></h2></div>", unsafe_allow_html=True)





options = [7, 14, 30, 60, 90, 120, 150, 180, 210, 240, 360, 720, 1440, 2880]
num_days = st.select_slider('Graph zoom (days)', options=options, value=120, key='slider1')
...
st.write(f"**Within the analyzed historical date (from {start_date} to now):**")
...
amount_stock = investment_value / price_paid
...
matrix_table = matrix_table.applymap(lambda x: (x*amount_stock) - 40)
...
with col3:
    ...
    st.image(filename)
with col4:
    options2 = [7, 14, 30, 60, 90, 120, 150, 180, 210, 240, 360, 720, 1440, 2880]
    num_days2 = st.select_slider('Graph zoom (days)', options=options2, key='slider2')

    # Filter the data based on the number of days selected by the user
    tickerDf = all_tickerDf.loc[pd.to_datetime(end_date) - pd.Timedelta(days=num_days2):end_date]
