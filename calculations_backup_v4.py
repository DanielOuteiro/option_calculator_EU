...
fig = go.Figure(data=go.Scatter(
    x=(merged_results_df_final['Investment Value']-40),
    y=basispreis_str,
    mode='markers',
    marker=dict(
        color=color_mapper,
...
# Display the row with the maximum Investment Value
st.write('Row with Maximum Investment Value:')
st.write(max_row)
...
price_range_matrix = range(target, int(current_price), 10)
...
for date in date_range_matrix:
    for price in price_range_matrix:
        url = f'https://api.onvista.de/api/v1/derivatives/{max_entity_value}/calculatorResult'
        params = {
...
price_paid = max_row['askPriceInstrument']
...
sns.heatmap(matrix_table, cmap='Greens', annot=True, fmt='.2f', annot_kws={"size": 7})
