...
target = st.sidebar.slider('Target', min_value=0, max_value=20000, value=15000)
...
sp500_data = yf.download("^GDAXI", start="1950-01-01")
...
if total_instances > 0:
    ...
    st.write((f"The DAX fell by {round(percent_fall, 2)}% or more within {days} days {count} times in history, which represents {percentage:.2f}% of all instances."))
else:
...
st.write(f"The maximum drop in the DAX that happened in {days} days is: {max_drop:.2f}% on {max_drop_date}. If that happened today, the DAX would fall to {woulfallto} ")
...
end_date = st.sidebar.date_input('End Date', value=pd.Timestamp('2023-02-25'))
date_range = format_date_range(start_date, end_date)

# Add the strike range pickers
strike_abs_start = st.sidebar.text_input('Strike Abs Start', value='14500')
strike_abs_end = st.sidebar.text_input('Strike Abs End', value='15000')
