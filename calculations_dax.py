...
start_date = st.sidebar.date_input('Load Historical data', value=pd.to_datetime('2000-01-01'))
...
vertical_date = (datetime.today() + timedelta(days=2)).date()
vertical_date = st.sidebar.date_input('Expiration Date', value=vertical_date)

target = st.sidebar.slider('Target', min_value=0, max_value=20000, value=14000)
...
if total_instances > 0:
    ...
    st.write(f"The DAX moved by {-(round(percent_fall, 2))}% or more within {days} or less days {count} times in history, which represents a {probability:.2%} probability")
else:
...
st.write(f"The average move in the DAX over {days} days is: {average_move:.2f}%")
...
ax.set_title(f"DAX Prices from {start_date} to {end_date}")
ax.set_xlabel("Date")
ax.set_ylabel("Price")
fig.autofmt_xdate()
...
st.write(f"The maximum drop in the DAX that happened in {days} days is: {max_drop:.2f}% on {max_drop_date}. If that happened today, the SP500 would fall to {woulfallto} ")
...
# Add the date range pickers
start_date = st.sidebar.date_input('Start Date', value=pd.Timestamp('2023-02-23'))
end_date = st.sidebar.date_input('End Date', value=pd.Timestamp('2023-02-24'))
date_range = format_date_range(start_date, end_date)

# Add the strike range pickers
strike_abs_start = st.sidebar.text_input('Strike Abs Start', value='14000')
strike_abs_end = st.sidebar.text_input('Strike Abs End', value='16000')
...
fig.update_layout(title='Scatter Plot', xaxis_title='Investment Value', yaxis_title='Price')
